...
- (void)updateDeviceReferences
{
	...
	{		
		if([[device groupName] isEqualToString:@"InternalBattery-0"])
		{
			mainDevice = device;
		}
		if (isiOS13Up)
		...
		else if([[device groupName] isEqualToString:@"Smart Battery Case"])
		{
			caseDevice = device;
		}
	}
...
{ 
	//main device
	if([[self groupName] isEqualToString:@"InternalBattery-0"])
	{
	...
	//case device 
	else if([[self groupName] isEqualToString:@"Smart Battery Case"])
	{
		%orig;
